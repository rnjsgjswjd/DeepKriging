...
for (i in 1:10){
  cat("Fold: ",i,"\n")
    ...
    geoR_fit = likfit(geodata_train, ini.cov.pars = c(18, 0.5), nugget=3, cov.model = 'spherical',
...
geoR_fit_OK = likfit(geodata_train, ini.cov.pars = c(18, 0.5), nugget=3,
                  cov.model = 'spherical', messages = FALSE)

krig_control_OK = krige.control(type.krige = "OK",  obj.model = geoR_fit_OK)

result_OK = krige.conv(geodata_train, locations = geodata_pred$coords, krige=krig_control_OK)
...
quilt.plot(us_metero$long,us_metero$lat,result_OK$predict,zlim=c(0,25),main="(d) Ordinary Kriging")

quilt.plot(BAUs_df$newdata.long, BAUs_df$newdata.lat, BAUs_df$newdata.p_mu, zlim=c(0,25),main="(f) FRK")
...
quilt.plot(us_metero$long,us_metero$lat,result$predict,zlim=c(0,25),main="Universal Kriging")
